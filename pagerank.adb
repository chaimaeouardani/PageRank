...
procedure Pagerank is
    ...
    --Initialiser les parametres par defaut
      ...
      procedure Free_mat is
		new Ada.Unchecked_Deallocation(T_Matrice, T_Matrice_access);
      ...
      begin
        ...
        if utiliser_matrice_creuse then
              put_line("L'implementation par les matrices creuses n'est pas encore disponible.L'exécution du programme continuera avec implementation naive");
        end if;
     ...
     begin
      ...
      procedure calculer_S(H : in out T_Matrice_access) is
      ...
      procedure Calculer_G(S : IN OUT T_Matrice_access) is
       begin
            for i in 0..N_pages-1 loop
                for j in 0..N_pages-1 loop
      ...
      --Recuperer les poids
      Function Poids_page(G : In T_Matrice_access ; N_iter : IN Integer ; N : IN integer) return T_td_access is
          poids : T_td_access;
          ...
          --calculer les iterations
       	  for k in 1..N_iter loop
              ...
              for j in 0..N-1 loop
                  somme := 0.0;
                  for i in 0..N-1 loop
                       somme := somme +  poids.all(i)* G.all(i,j);
                  end loop;
...
begin
    ...
    H := New T_Matrice(0..N_pages-1, 0..N_pages-1);
    --construire G
    calculer_S(H);
    calculer_G(H);
    --obtenir la liste des poids
    Poids := Poids_page(H , nb_iter , N_pages);
